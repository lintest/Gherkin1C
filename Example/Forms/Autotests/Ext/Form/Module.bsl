...
Процедура Тест_НесколькоСценариев(Ожидается) Экспорт
	...
	ДанныеФайла = Ожидается.Тест("Парсинг строки").Что(ВК).Функц("Прочитать", ТекстСценария).JSON().Вернуть();
	Ожидается.Тест("Сниппет первого шага контекста").Что(ДанныеФайла).Получить("background", "items", 0, "snippet").Равно("ястартуютесты");
	...
	Ожидается.Тест("Сниппет шага 1").Что(Сценарии).Получить(0, "items", 0, "snippet").Равно("яначинаюдвижение");
	Ожидается.Тест("Сниппет шага 2").Что(Сценарии).Получить(0, "items", 1, "snippet").Равно("яделаюшаг");
...
Процедура Тест_СтруктураСценария(Ожидается) Экспорт
	...
	Ожидается.Тест("Количество шагов").Что(НаборСтруктур).Получить(0, "items").Функц("Количество").Равно(2);
	...
	Ожидается.Тест("Содержание строк").Что(ТаблицаПримеров).Получить("body", 0, 0).Равно("""Книга""");
	
	Ожидается.Тест("Количество шагов").Что(НаборСтруктур).Получить(1, "items").Функц("Количество").Равно(3);
	...
	Ожидается.Тест("Содержание строк").Что(ТаблицаПримеров).Получить("body", 0, 0).Равно("2.5");
...
Процедура Тест_СканированиеПапки(Ожидается) Экспорт
	...
	Ожидается.Тест("Найденный файл").Что(ДанныеПапки).Получить(0, "filepath").Равно(ФайлНаРусском);
	
	ФильтрТегов = "{""include"":[""English""]}";
	ДанныеПапки = Ожидается.Тест(ФильтрТегов).Что(ВК).Функц("ПрочитатьПапку", ВременнаяПапка, ФильтрТегов).JSON().Вернуть();
	Ожидается.Тест("Количество файлов").Что(ДанныеПапки).Функц("Количество").Равно(2);
	Ожидается.Тест("Найденный файл").Что(ДанныеПапки).Получить(1, "filepath").Равно(ФайлНаАнглийском);
	Ожидается.Тест("Найденный файл").Что(ДанныеПапки).Получить(0, "filepath").Равно(ФайлEnglish);
