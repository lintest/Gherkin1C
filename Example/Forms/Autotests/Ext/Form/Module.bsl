...
Процедура Тест_РаботаСТаблицами(Ожидается) Экспорт
	...
	|		| Одна цифра         | 9                 |
	|		| Целое число        | 5416              |
	|		| Рациональное       | 8642,512          |
	|		| Отрицательное      | -786.54           |
	|		| Дата               | 12.02.2021        |
	|		| Дата               | 19/01/2018        |
	|		| Число в кавычках   | ""42365""         |
	|		| Спецсимволы        | \t\r\n            |
	|		| Табулятор          | \t                |
	|		| Перенос строки     | \n                |
	|		| Разделитель        | \|                |
...
Процедура Тест_ЭкспортныеСценарии(Ожидается) Экспорт
	...
	Ожидается.Тест("Третий шаг").Что(ШагиСниппета).Получить(2, "text").Сокр().Равно("Тогда я делаю звук громче");
