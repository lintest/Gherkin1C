...
Процедура Тест_РаботаСТаблицами(Ожидается) Экспорт
	...
	Гиперссылки = Ожидается.Тест("Гиперссылки").Что(ДанныеФайла).Получить("hyperlinks").Вернуть();
	Ожидается.Тест("Первая таблица ссылок").Что(Гиперссылки).Получить("tables", 0, "body").Функц("Количество").Равно(3);
	Ожидается.Тест("Вторая таблица ссылок").Что(Гиперссылки).Получить("steps", 0, "tables", 0, "body").Функц("Количество").Равно(2);
