...
Процедура Тест_ПередачаМногострочников(Ожидается) Экспорт
	...
	Ожидается.Тест("Третья строфа").Что(ШагиПодсценария).Получить(2, "snippet", "steps", 0, "multilines").Получить(0, "lines", 0, "text").Равно("Пусть Вторчермет гудит своей трубой.");
	Ожидается.Тест("Последняя стр").Что(ШагиПодсценария).Получить(2, "snippet", "steps", 0, "multilines").Получить(1, "lines", 0, "text").Равно("Проматерился, проревелся");
