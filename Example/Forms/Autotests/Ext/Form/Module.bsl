...
Процедура ЗаполнитьНаборТестов(ЮнитТест, Интерактивно = Ложь) Экспорт
	...
	ЮнитТест.Добавить("Тест_ЭкспортныеСценарииСтруктуры", "Экспортные сценарии в структуре");
...
&НаКлиенте
Процедура Тест_ЭкспортныеСценарииСтруктуры(Ожидается) Экспорт
	...
	|
	|Сценарий: Я делаю паузу 'Пауза'
	|	И пауза 'Пауза'
	|
	|Структура сценария: Сценарий с примерами
	|	И я делаю паузу <Длительность>
	|
	|	Примеры:
	|		|Длительность  |
	|		| 2            |
	...
	НаборСтруктур = Ожидается.Тест("Парсинг строки").Что(ВК).Функц("Прочитать", ТекстСценария).Получить("scenarios").Вернуть();
	Ожидается.Тест("Количество шагов").Что(НаборСтруктур).Получить(1, "steps").Функц("Количество").Равно(1);
	
//	НаборСтруктур[1].examples.body[0].script.steps[0].text
//	text	"   И я делаю паузу 2"	Строка	
	...
	ДанныеФайла = Ожидается.Тест("Сканирование файла с библиотеками").Что(ВК).Функц("ПрочитатьФайл", ОсновнойФайл, Бибилиотеки).JSON().Вернуть();
	Ожидается.Тест("Количество шагов").Что(НаборСтруктур).Получить(0, "steps").Функц("Количество").Равно(1);
