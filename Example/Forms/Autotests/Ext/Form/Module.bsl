...
Процедура Тест_СекцияПеременные(Ожидается) Экспорт
	...
	ФайлПодсценариев = ВременнаяПапка + "ФайлИмпорта.feature";
	ЗаписьТекста = Новый ЗаписьТекста(ФайлПодсценариев, КодировкаТекста.UTF8);
	ЗаписьТекста.Записать(ТекстСценария);
	ЗаписьТекста.Закрыть();
	...
	Ожидается.Тест("Путь импортируемого файла").Что(ДанныеФайла).Получить("variables", "import", 0, "path").Равно(ФайлПодсценариев);
	Ожидается.Тест("Имя импортируемого файла").Что(ДанныеФайла).Получить("variables", "import", 0, "name").Равно("ФайлИмпорта.feature");
