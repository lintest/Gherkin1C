...
Процедура Тест_ЭкспортныеСценарии(Ожидается) Экспорт
	...
	|	Путь ""Днем"" я слушаю радио ""Юность""
	...
	ДанныеПапки = Ожидается.Тест("Сканирование папки").Что(ВК).Функц("ПрочитатьПапку", ВременнаяПапка, ФильтрТегов).JSON().Вернуть();
	...
	Ожидается.Тест("Количество шагов подсценария").Что(ШагиСценария).Получить(0, "snippet", "steps").Функц("Количество").Равно(3);
	Ожидается.Тест("Сниппет первого шага").Что(ШагиСценария).Получить(0, "snippet", "steps", 0, "snippet").Равно("яберукнигу");
	Ожидается.Тест("Сниппет второго шага").Что(ШагиСценария).Получить(0, "snippet", "steps", 1, "snippet").Равно("яоткрываюстраницу");
